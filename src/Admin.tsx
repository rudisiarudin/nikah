      ...
      <div className="min-h-screen bg-neutral flex items-center justify-center p-4">
        ...
        >
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-primary text-neutral py-3 rounded-[10px] text-sm font-bold tracking-widest uppercase hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20">
              Masuk
            </button>
  ...
  return (
    <div className="min-h-screen bg-neutral text-primary sans-font">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Tabs */}
        <div className="flex gap-2 mb-8 bg-white/50 p-1.5 rounded-[10px] border border-primary/5 w-fit">
          <button
            onClick={() => setActiveTab('url')}
            className={`px-6 py-2.5 rounded-[10px] text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'url' ? 'bg-primary text-neutral shadow-md' : 'text-primary/60 hover:text-primary hover:bg-white/80'}`}
          >
            <LinkIcon size={16} /> Buat URL
          </button>
          <button
            onClick={() => setActiveTab('wishes')}
            className={`px-6 py-2.5 rounded-[10px] text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'wishes' ? 'bg-primary text-neutral shadow-md' : 'text-primary/60 hover:text-primary hover:bg-white/80'}`}
          >
        ...
        {activeTab === 'url' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white border border-primary/10 rounded-[10px] p-6 md:p-10 shadow-sm max-w-2xl">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full bg-neutral border border-primary/10 rounded-[10px] px-4 py-3 text-sm focus:outline-none focus:border-primary/40 focus:ring-2 focus:ring-primary/10 transition-all font-medium"
                />
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold tracking-wider uppercase text-primary/40 block">Kategori Ucapan</label>
                <div className="flex bg-neutral border border-primary/10 rounded-[10px] p-1.5 w-full font-medium sm:max-w-xs">
                  <button
                    onClick={() => setCategory('muslim')}
                    className={`flex-1 py-2.5 rounded-[10px] text-sm transition-all duration-300 ${category === 'muslim' ? 'bg-white shadow-sm text-primary font-bold' : 'text-primary/40 hover:text-primary'}`}
                  >
                    Muslim
                  </button>
                  <button
                    onClick={() => setCategory('umum')}
                    className={`flex-1 py-2.5 rounded-[10px] text-sm transition-all duration-300 ${category === 'umum' ? 'bg-white shadow-sm text-primary font-bold' : 'text-primary/40 hover:text-primary'}`}
                  >
                    Umum
                  </button>
                </div>
                  ...
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-4 pt-4">
                    ...
                    <div className="flex gap-3 grid-cols-2">
                      <button
                        ...
                        className="flex-1 bg-white border border-primary/20 text-primary py-3 rounded-[10px] text-xs font-bold tracking-widest uppercase hover:bg-primary/5 transition-all flex items-center justify-center gap-2"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 bg-[#25D366] text-white py-3 rounded-[10px] text-xs font-bold tracking-widest uppercase hover:bg-[#20bd5a] transition-all flex items-center justify-center gap-2 shadow-lg shadow-[#25D366]/30"
                      >
